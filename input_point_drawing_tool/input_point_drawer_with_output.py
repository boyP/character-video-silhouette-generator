"""
UI tool to automatically generates a new segmentation mask when an input point is selected
"""
...
# CHANGE ME
video_path = '/Users/pratikprakash/Documents/projects/video-silhouette-generator/input_videos/sueprocessed.mp4'
...
class VideoWriter:
    def __init__(self) -> None:
        ...
        output_path = 'sue480.mp4' # CHANGE ME
...
class SegmentAnything:
    ...
    def get_segmentation_mask(self, input_points_as_list, negative_points_as_list):
        combined_points_as_list = []

        for x in input_points_as_list:
            combined_points_as_list.append(x)
        for x in negative_points_as_list:
            combined_points_as_list.append(x)

        combined_points = np.array(combined_points_as_list)

        # Computes input_labels from input points
        input_labels = np.ones(len(input_points_as_list), dtype=int)
        input_labels2 = np.zeros(len(negative_points_as_list), dtype=int)
        combined_labels = np.append(input_labels, input_labels2)
...
class InputPointDrawer:
    def __init__(self, root, video) -> None:
        ...
        self.canvas.bind("<Button-2>", self.delete_dot)

        # Binding space key events
    ...
    def show_segmentation_output(self):
        ...
        negative_points = []
        for (x, y) in self.negative_points.values():
            (newX, newY) = convert_canvas_to_image_dimensions(x, y)
            negative_points.append([newX, newY])

        self.frame_with_mask = self.sam.get_segmentation_mask(input_points, negative_points)
    ...
    def load_frame(self):
        ...
        # Draw the input points from the previous frame when we load a new frame
        for (x, y) in list(self.current_points.values()):
            red_dot = self.draw_dot(x, y, self.dot_radius)
        ...
        # Draw the negative points from the previous frame when we load a new frame
        for (x, y) in list(self.negative_points.values()):
            blue_dot = self.draw_dot(x, y, self.dot_radius, color="blue")
            new_negative_points[blue_dot] = (x, y)

        # Reset the existing points with the new set of points we just drew so the ids match
        self.negative_points = new_negative_points
    ...
    def draw_dot(self, x, y, radius, color = "red"):
        return self.canvas.create_oval(x - radius, y - radius, x + radius, y + radius, fill=color, outline=color)
    
    def on_canvas_click(self, event):
        dot_x, dot_y = event.x, event.y

        if self.space_held:
            blue_dot = self.draw_dot(dot_x, dot_y, self.dot_radius, color="blue")
            self.negative_points[blue_dot] = (dot_x, dot_y)

        else:
            red_dot = self.draw_dot(dot_x, dot_y, self.dot_radius)
    ...
    def delete_dot(self, event):
        # Find the closest dot to the right-clicked position.
        ...
        elif closest_dot and closest_dot[0] in self.negative_points:
            self.canvas.delete(closest_dot[0])
            
            # Need to delete it from negative points too
            del self.negative_points[closest_dot[0]]
